...
FLOWER_BASIC_AUTH=admin:test

eval "celery -A run_celery.celery worker --loglevel=INFO -n worker-$ENV@%h --concurrency=1"$nooutput
eval "celery -A run_celery.celery flower --url_prefix=celery --basic_auth=$FLOWER_BASIC_AUTH --address=127.0.0.1 --port=$PORT"$nooutput
